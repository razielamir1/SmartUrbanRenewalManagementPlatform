...
-- ============================================================
-- ORDER MATTERS in PostgreSQL:
--   1. ENUM types
--   2. All tables (no RLS policies yet)
--   3. Helper functions (get_my_role needs users table to exist)
--   4. Enable RLS + add policies (policies may reference any table)
--   5. Triggers, storage
-- ============================================================
...
-- ─── 3. HELPER FUNCTIONS ─────────────────────────────────────
-- get_my_role() must come AFTER public.users exists.
-- All RLS policies use this to avoid recursive lookups.
...
-- Auto-create a public.users row on new signup
...
-- All tables and helper functions exist at this point.
...
CREATE POLICY "lpa_admin_manage"
  ON public.lawyer_project_assignments FOR ALL TO authenticated
  USING (public.get_my_role() = 'admin')
  WITH CHECK (public.get_my_role() = 'admin');

CREATE POLICY "lpa_lawyer_read_own"
  ON public.lawyer_project_assignments FOR SELECT TO authenticated
